...
- [x] Add a file path display to the markdown template (`markdown.html`) showing the path relative to the serve root
- [x] Add a file path display to the text template (`text.html`) in a similar location
- [x] Pass the display path from the handler to the template data (may need a new template field)
- [x] Style the path as a breadcrumb with clickable path segments linking to parent directories
- [x] Ensure the breadcrumb replaces or integrates with the existing `..` parent-nav link
