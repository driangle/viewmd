...
- [x] Create `python/` directory at repo root
- [x] Move `viewmd/` Python package into `python/viewmd/`
- [x] Move `tests/` into `python/tests/`
- [x] Move `pyproject.toml`, `requirements.txt`, `MANIFEST.in` into `python/`
- [x] Remove `viewmd.egg-info/` and `dist/` (build artifacts, not source)
- [x] Add `python/README.md` noting this is the archived PyPI-published Python version
- [x] Update root README.md with Go install instructions (`go install` or binary download)
- [x] Update CLAUDE.md with Go project description, commands, and architecture
